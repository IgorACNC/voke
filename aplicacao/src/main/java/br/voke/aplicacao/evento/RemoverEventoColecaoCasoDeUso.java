...
public class RemoverEventoColecaoCasoDeUso {

    private final ColecaoFavoritosServico servico;

    public RemoverEventoColecaoCasoDeUso(ColecaoFavoritosServico servico) {
        this.servico = Objects.requireNonNull(servico);
    }

    public ColecaoFavoritos executar(UUID colecaoId, UUID eventoId) {
        return servico.removerEvento(new ColecaoFavoritosId(colecaoId), eventoId);
    }
}
